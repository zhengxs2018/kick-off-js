...
export const MloStatsCard: React.FC<MloStatsCardProps> = ({ title, stat, onClick }) => {
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-bold text-zinc-900">{stat.total}</span>
        ...
        >
          {title.toLowerCase().includes('element') ? <Box size={24} /> : <Cpu size={24} />}
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {stat.details.slice(0, 3).map((detail, idx) => (
          ...
          >
            {detail.label} <span className="ml-1 text-zinc-400">({detail.count})</span>
          </span>
        ))}
