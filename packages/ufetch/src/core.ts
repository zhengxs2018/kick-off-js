...
export namespace Axios {
  ...
  export type RawRequestHeaderValue = string[] | string | number | null | undefined;
  type RawRequestHeaderValueFactory = () => MaybePromise<RawRequestHeaderValue>;

  export type RequestHeaderValue = RawRequestHeaderValue | RawRequestHeaderValueFactory;
  ...
  export interface RequestOptions extends Omit<RequestConfig, 'params' | 'headers'> {
    method: RequestMethod;
    headers: Record<string, string>;
  }

  export type ResponseType = 'arraybuffer' | 'blob' | 'json' | 'text' | 'stream' | 'formdata';
...
> {
  ...
  request<T = unknown>(config: Config): Promise<Axios.Response<Options, T, Response>>;
}
...
>(
  ...
  async function request(config: Config) {
    const options = await mergeRequestOptions<Defaults, Config, Options>(defaults, config);
